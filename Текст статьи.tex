...
\begin{equation}
	\begin{array}{l}
		...
		(r - \lambda_J \zeta) \frac{1}{2} S \frac{\partial F(S,V,\tau)}{\partial S} +
		\kappa(\theta - V) \frac{\partial2 F(S,V,\tau)}{\partial V} - 
		\smallskip\\
		(r-\lambda_J)F(S,V,\tau) +
		\lambda_J \int_0^\infty F(JS,V,\tau) f(J) dJ, S(\tau) > H
		\smallskip\\
		F(S,V,\tau) = 0,  S(\tau) \le H
	\end{array}
\end{equation}
где $f(J)$ - функция плотности вероятностей величины скачков $J$.
