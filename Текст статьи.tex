...
Введём также замену для процесса $S_t$, положив $Y_t = \ln(\frac{S_t}{H}) - \frac{\rho}{\sigma}V_t$. При этом $S_t = H\exp(Y_t + \frac{\rho}{\sigma}V_t) $  Такой вид замены одновременно позволяет перейти к логарифмической шкале и произвести нормировку по отношению к барьеру, которая является удобной с вычислительной точки зрения.
После введения обеих замен, система (1) примет вид:

\begin{equation}
\begin{array}{l}
d Y_t = (r-\frac{1}{2}V_t - \frac{\rho}{\sigma}\kappa(\theta - V_t) - \lambda_J \zeta)dt+\hat{\rho}\sqrt{V_t} dZ^S_t+\ln JdN_t,
\smallskip\\
...
Разделим промежуток $[0, T]$ на N равных частей, $\Delta \tau = \frac{T}{N}$. Зафиксируем точки $t_n = n \cdot \Delta T$. Построим  биномиальное дерево со <<склеенными>> вершинами, определяемыми по формуле:
\begin{equation*}	
V(n,k) = (\sqrt{V_0} + \frac{\sigma}{2}(2k-n)\sqrt{\Delta \tau})^2 \mathds{1}_{(\sqrt{V_0} + \frac{\sigma}{2}(2k-n)\sqrt{\Delta \tau}) > 0}, 
\\ n = 0,1,...,N, \; k = 0,1,...,n.
\end{equation*}
