...
\begin{equation}
\begin{array}{l}
d Y_t = (r-\frac{1}{2}V_t - \frac{\rho}{\sigma}\kappa(\theta - V_t) - \lambda_J \zeta)dt+\hat{\rho}\sqrt{V_t} dZ^S_t+lnJdN_t,
\smallskip\\
...
где $J$ имеет характеристическую экспоненту $\psi_j(\xi)$, для которой выполнено условие $\psi_j(-i) = 0$, а $j_i$ распределены так же, как $lnJ$. Обозначим $\mu_Y(V_t) = r - \frac{1}{2}V_t - \frac{\rho}{\sigma}\kappa(\theta - V_t)$ и $\mu_V(V_t) = \kappa(\theta - V_t)$.
...
Полагая $F_{N}=G(H\exp (Y_T+\frac{\rho}{\sigma}V_t ))$, получаем возможность записать:
