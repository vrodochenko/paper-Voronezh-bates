...
\begin{equation}
\begin{array}{l}
d Y_t = (r-\frac{1}{2}V_t - \frac{\rho}{\sigma}\kappa(\theta - V_t) - \lambda_J \zeta)dt+\hat{\rho}\sqrt{V_t} dZ^S_t+dlnJ_t,
\smallskip\\
dV_t= \kappa_V(\theta-V_t)dt+\sigma\sqrt{V_t}dZ^V_t,
\smallskip\\
j_t = \sum_i^{N_t}j_i
...
где $j_t$ имеет характеристическую экспоненту $\psi_j(\xi)$, для которой выполнено условие $\psi_j(0) = 0$, ${N_t}$ - пуассоновский процесс с интенсивностью $\lambda_J$, а $j_i$ распределены так же, как $lnJ$. Обозначим $\mu_Y(V_t) = r - \frac{1}{2}V_t - \frac{\rho}{\sigma}\kappa(\theta - V_t)$ и $\mu_V(V_t) = \kappa(\theta - V_t)$.
